...
- 127 is MAX_INT because 2^7 - 1
- SourceCode[0] is the number of lines within the source code.
- If the result is >127 overflow error
- Error 1 is an error with file name being incorrect(inputted file name 
  doesn't exist)
- Error 2 is any other error with the file
- Error Code 4 is a syntax error - missing colon (look in ExtractLabel)
- Error Code 5 is an error in opcodes
...
def LoadFile(SourceCode):
  """
- Source code is reset from previous run files
- File then opened, try and except used to error check for errors with the file
- Error 1 is an error with file name being incorrect(inputted file name doesn't exist)
- Error 2 is any other error with the file
  """
  FileExists = False
  ...
  if LineNumber > 0:
    print(SourceCode) ##testing what stuff looks life
...
def ExtractLabel(Instruction, LineNumber, Memory, SymbolTable):
  if len(Instruction) > 0: 
    ...
    if ThisLabel != EMPTY_STRING:
      if Instruction[5] != ':':
        print("Error Code 4") ## Error Code 4 = syntax error
        Memory[0].OpCode = "ERR"
      else:
...
def ExtractOperand(Instruction, LineNumber, Memory):
  if len(Instruction) >= 13:
    Operand = Instruction[12:]   ## don't understand this function
    ThisPosition = -1
    ...
    Operand = Operand.strip() #strip removes whitespace 
