...
all the rest are what we input
"""
...
def SolvePuzzle(PuzzleGrid, Puzzle, Answer, Solution):
  ...
  if PuzzleGrid[0][0] != 'X':
    ...
    print("Enter the letter 'H' for a hint: ")
    ...
    while CellInfo != EMPTY_STRING:
      ...
      if (len(CellInfo) != 3) and (CellInfo != "H"):
        ...
        Coordinate = input("Enter the Row and then Column of the box you want revealed: ")
        R = int(Coordinate[0])
        C = int(Coordinate[1])
        ...
        for row in range(1, GRID_SIZE + 1):
          for column in range(1, GRID_SIZE + 1):
            if PuzzleGrid[row][column] == SPACE:
              empty += 1
        print(f"There are {empty} cells left")
        print("Enter row column digit: ")
        print("Enter the letter 'H' for a hint: ")      
        print("(Press Enter to stop)")
        CellInfo = input()
...
def CalculateScore(Answer, ErrorCount):
  ...
  """
  Changes = int(Answer[2])
  Answer[1] = Changes - (ErrorCount*2)
  return Answer
...
def NumberPuzzle():
  """
       Parameters: None
       Return type: This is a procedure and doesnt return anything
       Description: Start of the code (top of hierarchy chart). Executes subroutine depending on the user's input. Prints statements based on the score and prints 5 random statements depending on the user's input for menuoption
  """
  Finished = False
  PuzzleGrid, Puzzle, Answer, Solution = ResetDataStructures()
  while not Finished:
