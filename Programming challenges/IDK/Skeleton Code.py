...
def DisplayErrorCode(ErrorNumber):
  if ErrorNumber == 1:
    print("Error code 1 - The piece has no possible moves")
  elif ErrorNumber == 2:
    print("Error code 2 - The piece cannot be moved there")
  elif ErrorNumber == 3:
    print("Error code 3 - Invalid row or column inputted")
...
def MoveDame(Board, Player, NewRow, NewColumn, OpponentsPieces):
  if Player == 'a':
    for i in [1, 3, 5, 7]:
      if Board[0][i] == SPACE: 
        NewColumn = i
  ...
  Take = input("Enter the piece ID of the opponent's piece you want to remove: ")
...
def CountNumberOfPieces(Player):
  ...
  for Piece in range(NUMBER_OF_PIECES):
    Number = Player[Piece][ROW]
    if Number >= 0:
      Count += 1
  return Count

    
def PrintResult(A, B, idk):
  NumA = CountNumberOfPieces(A)
  NumB = CountNumberOfPieces(B)
  MovesA = A[0][0]
  MovesB = B[0][0]
  DamesA = A[0][1]
  DamesB = B[0][1]
  ScoreA = MovesA - NumA - (DamesA * 10)
  ScoreB = MovesB - NumB - (DamesB * 10)
  if ScoreA < ScoreB:
    print(f"Player A is the winner with a score of {ScoreA}. Player B had a score of {ScoreB}.")
  elif ScoreB < ScoreA:
    print(f"Player B is the winner with a score of {ScoreB}. Player A had a score of {ScoreA}.")
  else:
    print(f"It is a draw both players got a score of {ScoreA} ")
